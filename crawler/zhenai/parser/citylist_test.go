...
func TestParseCityList(t *testing.T) {
	...
	if len(result.Requests) != resultSize {
		t.Errorf("result should have %d "+"requests; bug has %d", resultSize, len(result.Requests))
	}
	...
	if len(result.Items) != resultSize {
		t.Errorf("result should have %d "+"requests; bug has %d", resultSize, len(result.Items))
	}
